    ...
    def load_data_tacred_arnor_ner(self, data_file, rel2id_file, load_ner=True):
        ...
        for item in data:
            ...
            if load_ner:
                ...
                ner = None
            instances.append(
                Instance(words, relation, head, tail, head_pos, tail_pos, head_type, tail_type, d_rel=d_rel, ner=ner))
        ...
        for ins in instances[:5]:
            print("words: {}, head: {}, head_pos: {}, tail: {}, tail_pos: {}, relation: {}, d_rel: {}, ner: {}"
                  .format(" ".join(ins.words), ins.head, str(ins.headpos), ins.tail, str(ins.tailpos), ins.relation,
                          ins.d_rel, ins.ner))
    ...
    def preprocess(self, data, token_mask_id=0):
        ...
        for idx, item in enumerate(data):
            ...
            input_ids, unk_num = self.tokenizer.convert_tokens_to_ids(tokens, self.max_len,
                                                             self.tokenizer.vocab['[PAD]'], self.tokenizer.vocab['[UNK]'], uncased=False)
